...
//    ilist=new text_field(x,y,NET_PORT,symbol_str("use_port"),"******",port,ilist);
    ...
    ilist=new
text_field(x,y,NET_SERVER_NAME,symbol_str("server_name"),"*********************************",game_name,ilist); ilist=new
text_field(x,y,NET_NAME,symbol_str("your_name"),"****************",name,ilist);
  }
...
int net_configuration::get_options(int server)
{
  ...
  {
    list = center_ifield(
        new text_field(x, y + 30, NET_NAME, symbol_str("your_name"), "************************", name, list), x,
        x + ns_w, NULL);
    list = center_ifield(
        new text_field(0, 0, NET_SERVER_NAME, symbol_str("server_name"), "************************", game_name, list),
        x, x + ns_w, list);
    ...
    q = new button(0, 0, LVL_2, symbol_str("lvl_2"), NULL);
    q->push();
    b->add_button(q);
    ...
    list = center_ifield(new text_field(0, 0, NET_KILLS, symbol_str("kills_to_win"), "***", "25", list), x, x + ns_w,
                         list);
  }
  else
  {
