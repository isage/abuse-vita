...
public:
  ...
  backtile *get_bg(int x)
  {
    if (x < 0 || x >= nbacktiles || backtiles[x] < 0)
      return cache.backt(backtiles[BLACK]);
    else
      return cache.backt(backtiles[x]);
  }
  foretile *get_fg(int x)
  {
    if (x < 0 || x >= nforetiles || foretiles[x] < 0)
      return cache.foret(foretiles[BLACK]);
    else
  ...
  void show_time();
  tile_type GetMapBg(ivec2 pos)
  {
    return current_level->GetBg(pos);
  }
  tile_type GetMapFg(ivec2 pos)
  {
