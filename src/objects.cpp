...
int simple_object::total_vars()
{
  return TOTAL_OBJECT_VARS;
}

obj_desc object_descriptions[TOTAL_OBJECT_VARS]
    = {{"fade_dir", RC_8}, {"frame_dir", RC_8},     {"direction", RC_8}, {"gravity_on", RC_8}, {"fade_count", RC_8},

       {"fade_max", RC_8}, {"active", RC_8},        {"flags", RC_8},     {"aitype", RC_8},     {"xvel", RC_32},

       {"fxvel", RC_8},    {"yvel", RC_32},         {"fyvel", RC_8},     {"xacel", RC_32},     {"fxacel", RC_8},

       {"yacel", RC_32},   {"fyacel", RC_8},        {"x", RC_32},        {"fx", RC_8},         {"y", RC_32},

       {"fy", RC_8},       {"hp", RC_16},           {"mp", RC_16},       {"fmp", RC_16},       {"cur_frame", RC_16},

       {"aistate", RC_16}, {"aistate_time", RC_16}, {"targetable", RC_8}};
  ...
  if (figures[otype]->get_fun(OFUN_AI))
  {
    ...
    if (keep_ai_info())
    {
      if (aistate() != old_aistate)
        ...
        set_aistate_time(aistate_time() + 1);
    }
    if (!NILP(ret))
      return 1;
    else
...
void game_object::damage_fun(int amount, game_object *from, int32_t hitx, int32_t hity, int32_t push_xvel,
                             int32_t push_yvel)
{
  if (!hurtable() || !alive())
    return;
  ...
  if (c && hp() <= 0)
  {
    ...
    if (v)
      v->kills++; // attack from another player?
...
void game_object::draw_trans(int count, int max)
{
  ...
  cpict->PutFade(main_screen,
                 ...
                 count, max, color_table, the_game->current_palette());
}
...
void game_object::draw_tint(int tint_id)
{
  ...
  if (fade_count())
    cpict->PutFadeTint(main_screen,
                       ivec2((direction < 0 ? x - (cpict->Size().x - x_center() - 1) : x - x_center()) - current_vxadd,
                             y - cpict->Size().y + 1 - current_vyadd),
                       fade_count(), fade_max(), cache.ctint(tint_id)->data, color_table, the_game->current_palette());
