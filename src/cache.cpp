...
void CacheList::unmalloc(CacheItem *i)
{
  switch (i->type)
  {
    ...
    case SPEC_CHARACTER:
      delete ((figure *)i->data);
      break;
    case SPEC_FORETILE:
      delete ((foretile *)i->data);
      break;
    case SPEC_BACKTILE:
      delete ((backtile *)i->data);
      break;
    case SPEC_IMAGE:
      delete ((image *)i->data);
      break;
    case SPEC_EXTERN_SFX:
      delete ((sound_effect *)i->data);
      break;
    case SPEC_PARTICLE:
      delete ((part_frame *)i->data);
      break;
    case SPEC_EXTERNAL_LCACHE:
      if (i->data)
        free(i->data);
      break;
    case SPEC_PALETTE:
      delete ((char_tint *)i->data);
      break;
    default:
        ...
        for (i = 0; i < tsaved; i++)
        {
          ...
          if (j != -1)
          {
            if (list[j].last_access < 0) // if not loaded
              ...
              list[j].last_access = 2; // mark as loaded and needing to stay that way
            priority[i] = j;
            tmatches++;
          }
        }
...
int CacheList::reg_object(char const *filename, LObject *object, int type, int rm_dups)
{
...
void CacheList::show_accessed()
{
  ...
  do
  {
    ...
    if (new_old)
    {
      ci  = new_old;
      old = ci->last_access;
      printf("type=(%20s) file=(%20s) access=(%6ld)\n", spec_types[ci->type], crc_manager.get_filename(ci->file_number),
