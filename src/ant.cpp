...
static int ant_congestion(game_object *o)
{
  ...
  {
    if (d->otype == o->otype && abs(o->x - d->x) < 30 && abs(o->x - d->y) < 20)
      return 1;
  }
  return 0;
}
...
{
  int32_t firex = o->x + (o->direction > 0 ? 15 : -15), firey = o->y - 15, playerx = b->x + b->xvel() * 8,
          playery = b->y - 15 + b->yvel() * 2;
  ...
  switch (o->aistate())
  {
    ...
    case ANT_RUNNING:
    {
      ...
      if (!ant_dodge(o))
      {
        if ((o->x > b->x && o->direction == -1) || (o->x < b->x && o->direction == 1))
        {
          ...
          {
            int32_t xm
                = o->direction > 0 ? get_ability(o->otype, run_top_speed) : -get_ability(o->otype, run_top_speed);
    ...
    case ANT_FIRE:
    {
      if (!ant_dodge(o))
      {
        if (o->state == S_fire_wait)
        {
          ...
          o->set_state((character_state)S_fire_wait);
      }
    }
    break;
    case ANT_JUMP_ROOF:
    ...
    case ANT_ROOF_WALK:
    {
      if (player_list->next)
        ...
        b = player_list->m_focus;
      scream_check(o, b);
      if (((jrand() % 8) == 0 && abs(o->x - b->x) < 10 && o->y < b->y) || o->lvars[ANT_need_to_dodge] == 1)
      {
        o->set_gravity(1);
      ...
      else
      {
        ...
        {
          int speed = o->direction > 0 ? get_ability(o->otype, run_top_speed) : -get_ability(o->otype, run_top_speed);
...
void show_stats()
{
  ...
  if (current_level)
  {
    ...
    blank.clear();
    wm->SetMouseShape(blank.copy(), ivec2(0, 0));
    main_screen->clear();
    ...
    image *im = NULL;
    if (rand() % 2)
      im = cache.img(cache.reg("art/frame.spe", "end_level_screen", SPEC_IMAGE, 1));
    else
      im = cache.img(cache.reg("art/fore/endgame.spe", "tbc", SPEC_IMAGE, 1));
