...
int get_remote_lsf(net_address *addr, char *filename) // filename should be 256 bytes
{
  if (prot)
  {
    ...
    if (sock->write(&ctype, 1) != 1 || sock->read(&len, 1) != 1 || len == 0 || sock->read(filename, len) != len)
    {
      delete sock;
      return 0;
    }
...
int request_server_entry()
{
  if (prot && main_net_cfg)
  {
    ...
    game_sock = prot->create_listen_socket(main_net_cfg->port + 2,
                                           net_socket::SOCKET_FAST); // this is used for fast game packet transmission
    if (!game_sock)
    {
      if (comm_sock)
        delete comm_sock;
    ...
    if (sock->write(&len, 1) != 1 || sock->write(uname, len) != len || sock->write(&port, 2) != 2
        ||                           // send server out game port
        sock->read(&port, 2) != 2 || // read server's game port
        sock->read(&nkills, 2) != 2 || sock->read(&cnum, 2) != 2
        || cnum == 0 // read player number (cannot be 0 because 0 is server)
    )
    {
      delete sock;
      return 0;
...
void net_reload()
{
  if (prot)
  {
    if (net_server)
    {
      ...
      do
      { // make sure server saves the file
    ...
    else if (current_level)
    {
      ...
      while (join_list)
      {
        ...
        for (; f && f->next; f = f->next)
          ; // find last player, add one for pn
        int i, st = 0;
      ...
      base->input_state
          = INPUT_RELOAD; // if someone missed the game tick with the RELOAD data in it, make sure the get it
  ...
  if (prot && base->input_state != INPUT_PROCESSING) // if input is not here, wait on it
  {
    ...
    while (base->input_state != INPUT_PROCESSING)
    {
      ...
      if (now.diff_time(&start) > 0.05)
      {
        ...
        {
          abort = wm->CreateWindow(
              ivec2(0, yres / 2), ivec2(-1, wm->font()->Size().y * 4),
...
int become_server(char *name)
{
  if (prot && main_net_cfg)
  {
    ...
    if (comm_sock)
      delete comm_sock;
    comm_sock = prot->create_listen_socket(main_net_cfg->port,
                                           net_socket::SOCKET_SECURE); // this is used for incomming connections
    if (!comm_sock)
    {
      prot = NULL;
      return 0;
    ...
    if (game_sock)
      delete game_sock;
    game_sock = prot->create_listen_socket(main_net_cfg->port + 1,
                                           net_socket::SOCKET_FAST); // this is used for fast game packet transmission
    if (!game_sock)
    {
      if (comm_sock)
