...
public:
  ...
  game_object *first_object()
  {
    return first;
  }
  game_object *first_active_object()
  {
    return first_active;
  }
  uint16_t foreground_width()
  {
    return fg_width;
  }
  uint16_t foreground_height()
  {
    return fg_height;
  }
  ...
  int fg_raised(int x, int y)
  {
    CHECK(x >= 0 && y >= 0 && x < fg_width && y < fg_height);
    return (*(map_fg + x + y * fg_width)) & 0x4000;
  }
  void fg_set_raised(int x, int y, int r)
  {
    CHECK(x >= 0 && y >= 0 && x < fg_width && y < fg_height);
    uint16_t v = (*(map_fg + x + y * fg_width)) & (0xffff - 0x4000);
    if (r)
      (*(map_fg + x + y * fg_width)) = v | 0x4000;
    else
      (*(map_fg + x + y * fg_width)) = v;
  }
  ...
  void clear_fg(int32_t x, int32_t y)
  {
    *(map_fg + x + y * fg_width) &= 0x7fff;
  }

  uint16_t *get_fgline(int y)
  {
    CHECK(y >= 0 && y < fg_height);
    return map_fg + y * fg_width;
  }
  uint16_t *get_bgline(int y)
  {
    CHECK(y >= 0 && y < bg_height);
    return map_bg + y * bg_width;
  }
  uint16_t GetFg(ivec2 pos)
  {
    if (pos >= ivec2(0) && pos < ivec2(fg_width, fg_height))
      return fgvalue(*(map_fg + pos.x + pos.y * fg_width));
    else
      return 0;
  }
  uint16_t GetBg(ivec2 pos)
  {
    if (pos >= ivec2(0) && pos < ivec2(bg_width, bg_height))
      return *(map_bg + pos.x + pos.y * bg_width);
    else
      return 0;
  }
  void PutFg(ivec2 pos, uint16_t tile)
  {
    *(map_fg + pos.x + pos.y * fg_width) = tile;
  }
  void PutBg(ivec2 pos, uint16_t tile)
  {
  ...
  void hurt_radius(int32_t x, int32_t y, int32_t r, int32_t m, game_object *from, game_object *exclude, int max_push);
