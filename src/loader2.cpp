...
void load_tiles(Cell *file_list)
{
  ...
  Cell *fl;
  int old_fsize = nforetiles, old_bsize = nbacktiles;
...
void load_data(int argc, char **argv)
{
  ...
  ERROR(big_font_pict != -1 || small_font_pict != -1, "No font loaded (use load_big_font or load_small_font)!");
