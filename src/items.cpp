...
boundary::boundary(bFILE *fp, char const *er_name)
    : point_list(fp)
{
...
boundary::boundary(boundary *p)
    : point_list(p->tot, p->data)
{
  int x1, y1, x2, y2, checkx, checky, i;
  uint8_t *point_on;
  if (tot)
...
foretile::foretile(bFILE *fp)
{
  ...
  int r[AUTOTILE_WIDTH * AUTOTILE_HEIGHT], g[AUTOTILE_WIDTH * AUTOTILE_HEIGHT], b[AUTOTILE_WIDTH * AUTOTILE_HEIGHT],
  ...
  for (l = 0; l < AUTOTILE_WIDTH * AUTOTILE_HEIGHT; l++)
    micro_image->PutPixel(
        ivec2(l % AUTOTILE_WIDTH, l / AUTOTILE_WIDTH),
        color_table->Lookup((r[l] / (t[l] * 4 / 5)) >> 3, (g[l] / (t[l] * 4 / 5)) >> 3, (b[l] / (t[l] * 4 / 5)) >> 3));
...
{
  return forward->DiskUsage() + backward->DiskUsage() + hit->size() + f_damage->size() + b_damage->size()
         + sizeof(figure);
}
