...
int text_draw(int y, int x1, int y1, int x2, int y2, char const *buf, JCFont *font, uint8_t *cmap, char color)
{
  ...
  while (buf && *buf)
  {
    do
    {
      ...
      word_start = buf;
      for (word_len = 0, word_start = buf, word_size = 0;
           *buf && *buf != ' ' && *buf != '\r' && *buf != '\n' && *buf != '\t' && (*buf != '\\' || buf[1] != 'n');
           buf++, word_size += w, word_len++)
        ;
...
void director::wait(void *arg)
{
  ...
  int done                          = 0;
  LSymbol *pan_symbol = LSymbol::FindOrCreate("pan"), *text_symbol = LSymbol::FindOrCreate("text");
  ...
  do
  {
    ...
    else if (arg == pan_symbol)
      done = 1;

    if (text)
    {
      if (text_draw(text_y, the_game->first_view->m_aa.x + tleft, the_game->first_view->m_aa.y + ttop,
                    the_game->first_view->m_bb.x - tright, the_game->first_view->m_bb.y - tbottom, text, font,
                    white_light + 32 * 256, wm->bright_color()
