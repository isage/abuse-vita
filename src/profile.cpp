...
void profile_init()
{
  ...
  prof_win = wm->CreateWindow(ivec2(prop->getd("profile x", -1), prop->getd("profile y", -1)),
                              ivec2(20, prof_height + 1) * console_font->Size(), NULL, "PROFILE");
}
...
{
  profile_sort();
  if (prof_list[0].total_time <= 0.0)
    return; // nothing took any time!
  ...
  prof_win->m_surf->Bar(ivec2(0, prof_win->y1()), ivec2(prof_win->m_surf->Size().x - 1, prof_win->m_surf->Size().y), 0);
  int dy = 0;
  for (; i < prof_height; i++)
  {
    ...
    prof_win->m_surf->Bar(ivec2(spliter - 1 - (int)(prof_list[i].total_time * time_scaler), dy + 1),
                          ivec2(spliter - 1, dy + console_font->Size().y - 1), wm->bright_color());
