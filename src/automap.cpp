...
void automap::draw()
{
  ...
  long sx, ex, sy, ey, x, y, window_xstart, window_ystart, window_xend, window_yend, draw_xstart, draw_ystart, i, j;
  ...
  screen->Bar(ivec2(window_xstart, window_ystart), ivec2(draw_xstart, window_yend), 0);
  screen->Bar(ivec2(window_xstart, window_ystart), ivec2(window_xend, draw_ystart), 0);
  ...
  screen->AddDirty(ivec2(window_xstart, window_ystart), ivec2(window_xend + 1, window_yend + 1));
  ...
  screen->SetClip(ivec2(window_xstart, window_ystart), ivec2(window_xend + 1, window_yend + 1));
#if 0
  ...
  for (j = draw_ystart, y = sy; y <= ey; j += AUTOTILE_HEIGHT, y++)
  {
    ...
    for (i = draw_xstart, x = sx; x <= ex; i += AUTOTILE_WIDTH, x++, fgline++)
    {
      if ((*fgline) & 0x8000)
      {
        ...
        screen->Bar(ivec2(i, j), ivec2(i + AUTOTILE_WIDTH - 1, j + AUTOTILE_HEIGHT - 1), 0);
    }
  }
...
void automap::toggle_window()
{
  ...
  else
  {
    ...
    automap_window = wm->CreateWindow(ivec2(0), ivec2(w * AUTOTILE_WIDTH, h * AUTOTILE_HEIGHT), NULL, "Map");
    automap_window->m_surf->Bar(ivec2(17, 1), ivec2(17 + 8 * 6 + 3, 6), wm->medium_color());
    wm->font()->PutString(automap_window->m_surf, ivec2(20, 2), "Automap", wm->dark_color());
    draw();
  }
}
