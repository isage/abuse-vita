...
class game_object : public simple_object
{
  ...
  {
    return figures[otype]->get_sequence(state);
  }
...
public:
  ...
  int hurt_all()
  {
    return figures[otype]->get_cflag(CFLAG_HURT_ALL);
  }
  int stoppable()
  {
    return figures[otype]->get_cflag(CFLAG_STOPPABLE);
  }
  int can_block()
  {
    return figures[otype]->get_cflag(CFLAG_CAN_BLOCK);
  }

  int hurtable()
  {
    return figures[otype]->get_cflag(CFLAG_HURTABLE);
  }
  int pushable()
  {
    return figures[otype]->get_cflag(CFLAG_PUSHABLE);
  }
  ...
  void receive_signal(long signal)
  {
    (void)signal;
  }
  int push_range();
  ...
  int has_sequence(character_state s)
  {
    return figures[otype]->has_sequence(s);
  }
  ...
  TransImage *picture()
  {
    return current_sequence()->get_frame(current_frame, direction);
  }
  ...
  {
    return current_sequence()->get_figure(current_frame);
  }
  int total_frames()
  {
  ...
  void set_aimemory(game_object *p)
  {
    add_object(p);
    p->set_flags(p->flags() | KNOWN_FLAG);
  }
  int alive()
  {
    if (state == dead || hp() <= 0)
...
public:
  game_object *me;
  object_node *next;
  object_node(game_object *Me, object_node *Next)
  {
    me   = Me;
    next = Next;
  }
