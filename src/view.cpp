...
void view::add_ammo(int weapon_type, int total)
{
  ...
  if (weapons[weapon_type] == -1)
    return; // don't have weapon yet, can't give ammo
...
void set_local_players(int total)
{
  ...
  for (view *f = player_list; f; f = f->next)
  {
    ...
    {
      view *n = last->next;
      while (n && !n->local_player())
        n = n->next; // find next local player
...
int32_t view::get_view_var_value(int num)
{
  switch (num)
  {
    ...
    case V_Y_SUGGESTION:
      return y_suggestion;
      break;
    case V_B1_SUGGESTION:
      return b1_suggestion;
      break;
    case V_B2_SUGGESTION:
      return b2_suggestion;
      break;
    case V_B3_SUGGESTION:
      return b3_suggestion;
      break;
    case V_B4_SUGGESTION:
      return b4_suggestion;
      break;
