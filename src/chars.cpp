...
char const *state_names[MAX_STATE]
    = {"dead",         "dieing",    "stopped",     "start_run_jump", "run_jump", "run_jump_fall",
       "end_run_jump", "flinch_up", "flinch_down", "morph_pose",     "running"
#if 0
...
char const *cflag_names[TOTAL_CFLAGS]
    = {"hurt_all",   "is_weapon", "stoppable", "can_block",     "hurtable",       "pushable",
       "unlistable", "add_front", "cached_in", "need_cache_in", "unactive_shield"};

char const *ofun_names[TOTAL_OFUNS]
    = {"ai_fun",   "move_fun",    "draw_fun",   "map_draw_fun",       "damage_fun",     "next_state_fun",
       "user_fun", "constructor", "reload_fun", "get_cache_list_fun", "type_change_fun"};
    ...
    if (index < tiv)
    {
      if (vars[index])
      {
        lbreak("While defining object %s :\n"
               ...
               lstring_value(((LSymbol *)name)->GetName()), lstring_value(((LSymbol *)symbol)->GetName()), index,
               lstring_value(((LSymbol *)name)->GetName()), lstring_value(((LSymbol *)vars[index])->GetName()),
...
CharacterType::CharacterType(LList *args, LSymbol *name)
{
  ...
  for (; field; field = CDR(field))
  {
    ...
    else if (f == l_states)
    {
      ...
      PtrRef r4(l);
      char fn[100];
      strcpy(fn, lstring_value(CAR(l)->Eval()));
      l = CDR(l);
      while (l)
      {
        int index;
