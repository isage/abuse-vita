...
void morph_char::draw(game_object *who, view *v)
{
  if (fleft)
  {
    ivec2 pos = the_game->GameToMouse(ivec2(who->x - (cx >> 16), who->y - (cy >> 16)), v);
...
morph_char::morph_char(game_object *who, int to_type, void (*stat_fun)(int), int anneal, int frames)
{
  ...
  if (!t1->has_sequence(morph_pose) || t1->morph_mask < 0 || !t2->has_sequence(morph_pose) || t2->morph_mask < 0)
    fleft = 0;
  else
  {
    if (anneal == -1)
    ...
    if (sm->t)
    {
      delete h1;
      delete h2;
      figure *f1 = t1->get_sequence(morph_pose)->get_figure(0), *f2 = t2->get_sequence(morph_pose)->get_figure(0);
      image *i1 = f1->forward->ToImage(), *i2 = f2->forward->ToImage();
