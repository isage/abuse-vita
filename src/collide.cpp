...
void add_collide(collide_patch *&first, int32_t x1, int32_t y1, int32_t x2, int32_t y2, game_object *who)
{
  collide_patch *next;
        ...
        if (subject->can_hurt(target)) // see if we can hurt him before calculating
        {
          ...
          for (i = (int)s_hit->tot - 1; i > 0 && !rec; i--)
          {
            for (t_dat = t_damage->data, j = (int)t_damage->tot - 1; j > 0 && !rec; j--)
            {
              ...
              xp1 = target->x + target->tx(*t_dat);
              t_dat++;
              yp1 = target->y + target->ty(*t_dat);
              t_dat++;
