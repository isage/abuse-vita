...
public:
  ...
  int x_suggestion, // input from the player at the current time
      y_suggestion, b1_suggestion, b2_suggestion, b3_suggestion, b4_suggestion, pointer_x, pointer_y, freeze_time;
  ...
  int has_weapon(int weapon_type)
  {
    return (weapons[weapon_type] != -1);
  }
  ...
  int weapon_changed()
  {
    return suggest.send_weapon_change;
  }
