...
tile_picker::tile_picker(int X, int Y, int ID, int spec_type, int Scale, int scroll_h, int Wid, ifield *Next)
...
void tile_picker::scroll_event(int newx, image *screen)
{
  ...
  for (int i = newx; i < newx + th * wid; i++)
  {
        ...
        case SPEC_FORETILE:
        {
          ...
          else
          {
            ...
            if (rev)
            {
              ...
              scale_put(&im, screen, xyo.x, xyo.y, xw, ya);
          }
        }
        break;
        case SPEC_BACKTILE:
    ...
    if (i == c)
      screen->Rectangle(m_pos, m_pos + ivec2(xw - 1, ya - 1), wm->bright_color());
  }
}
