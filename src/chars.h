...
enum
{
  OFUN_AI,    // objects ai function called by the mover, should call (tick)
...
public:
  ...
  uint16_t cflags;
  void *get_fun(int name)
  {
    return fun_table[name];
  }
  int get_cflag(int name)
  {
    return cflags & (1 << name);
  }
  void set_cflag(int name, int x)
  {
    if (x)
      cflags |= (1 << name);
    else
