...
size_t block_size(LObject *level) // return size needed to recreate this block
{
  ...
  switch (item_type(level))
  {
    case L_CONS_CELL:
    {
      size_t ret = sizeof(unsigned char) + sizeof(uint);
      void *b    = level;
    ...
    case L_STRING:
      return sizeof(unsigned char) + sizeof(uint) + strlen(lstring_value(level)) + 1;
    case L_NUMBER:
      return sizeof(unsigned char) + sizeof(uint);
    case L_SYMBOL:
      return sizeof(unsigned char) + sizeof(size_t);
  }
  ...
  switch (type)
  {
    case L_CONS_CELL:
      ...
      else
      {
         ...
         * to reflect that. */
        fp->write_uint32(b ? -(int)count : count);
        if (b)
    ...
    case L_SYMBOL:
    {
      size_t p = (size_t)level;
      for (size_t i = 0; i < sizeof(size_t); i++)
      {
        fp->write_uint8((unsigned char)p);
        p >>= 8;
      }
    }
  }
