...
#include <fcntl.h>
#include <stdlib.h>
#include <SDL2/SDL_config.h>//AR (#include <SDL2/SDL_config.h>)
#include <string.h>
...
class bFILE     // base file type which other files should be derived from (jFILE & NFS for now)
{
  ...
  virtual ~bFILE();
    ...
    double read_double();
    void write_uint16(unsigned short x);
    void write_uint32(unsigned int x);
    void write_uint8(unsigned char x);
...
{
public:
    spec_entry(unsigned char spec_type, char const *object_name,
...
void write_uint16(FILE *fp, unsigned short x);
void write_uint32(FILE *fp, unsigned int x);
void write_uint8(FILE *fp, unsigned char x);
