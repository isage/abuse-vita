...
public:
  ...
  sequence(int *figures, int total_frames)
  {
    total = total_frames;
    seq   = figures;
  }
  ...
  figure *get_figure(short current)
  {
    return cache.fig(seq[current]);
  }
  int cache_in();
  int x_center(short current)
  {
    return (short)(cache.fig(seq[current])->xcfg);
  }
  int length()
  {
    return total;
  }
  int get_advance(int current)
  {
    return cache.fig(seq[current])->advance;
  }
  size_t MemUsage();
  ~sequence();
};
