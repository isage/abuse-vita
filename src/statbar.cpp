...
void status_bar::area(int &x1, int &y1, int &x2, int &y2)
{
  ...
  // status bar width & height
  int sb_w = sb->Size().x, sb_h = sb->Size().y;
...
void status_bar::draw_ammo(image *screen, int weapon_num, int amount, int light)
{
  if (total_weapons)
  {
    int x1, y1, x2, y2;
    area(x1, y1, x2, y2);
    draw_num(screen, x1 + (small_render ? 52 * 2 + weapon_num * 34 * 2 : 52 + weapon_num * 34),
  ...
  if (total_weapons && v)
  {
    if (DEFINEDP(symbol_value(l_mouse_can_switch)) && symbol_value(l_mouse_can_switch) && mouse_in_area())
    {
      if ((current_level->tick_counter() & 4) == 0)
        ...
        wm->SetMouseShape(cache.img(c_mouse2)->copy(), ivec2(4, 4));
      changed_cursor = 1;
    }
