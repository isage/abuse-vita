...
signed char small_fire_off[24 * 2] = // x & y offset from character to end of gun.
    {17,  20,                        // 1
     ...
     17,  28,  15,  33, 11,  39,     // 5
     7,   43,  -3,  44,              // 7
     -10, 42,  -16, 39, -20, 34, -20, 28, -20, 25, -19, 20, -19, 16, -16, 14, -14,
     11,  -11, 9,   -7, 8,   -3, 8,   2,  8,   6,  9,   10, 10,  14, 13,  16, 15};

signed char large_fire_off[24 * 2]
    = {18,  25, 17,  30, 15,  34, 14,  36, 10, 39, 7,  41, 4, 42, -3, 41, -8, 39, -11, 37, -14, 33, -16, 30,
       -18, 25, -17, 21, -14, 17, -11, 15, -7, 13, -4, 12, 3, 12, 9,  12, 12, 15, 14,  16, 15,  18, 16,  21};
...
void *laser_ufun(void *args)
{
  ...
  if (signal == l_FIRE)
  {
    if (!o->lvars[fire_delay1]) // make sur we are not waiting of previous fire
    {
      ...
      ret = LNumber::Create(0); // can't fire yet, return 0 ammo subtract
  }
  return ret;
}
...
static int player_move(game_object *o, int xm, int ym, int but)
{
  if (!o->lvars[in_climbing_area])
  {
    ...
    return climb_handler(o, xm, ym, but);
}
  ...
  if (o->aistate() > 0)
  {
    switch (o->lvars[special_power])
    {
      ...
      case HEALTH_POWER:
      {
        ...
        if (o->controller() && o->controller()->local_player())
          main_screen->PutImage(cache.img(S_health_image),
                                ivec2(o->controller()->m_bb.x - 20, o->controller()->m_aa.y + 5), 1);
      }
      break;
      case FAST_POWER:
      {
        ...
        {
          case stopped:
            o->state = (character_state)S_fast_stopped;
            break;
          case running:
            o->state = (character_state)S_fast_running;
            break;
          case start_run_jump:
            o->state = (character_state)S_fast_start_run_jump;
            break;
          case run_jump:
            o->state = (character_state)S_fast_run_jump;
            break;
          case run_jump_fall:
            o->state = (character_state)S_fast_run_jump_fall;
            break;
          case end_run_jump:
            o->state = (character_state)S_fast_end_run_jump;
            break;
          default:
            break;
        }
      ...
      case FLY_POWER:
      {
        ...
        {
          case stopped:
            o->state = (character_state)S_fly_stopped;
            break;
          case running:
            o->state = (character_state)S_fly_running;
            break;
          case start_run_jump:
            o->state = (character_state)S_fly_start_run_jump;
            break;
          case run_jump:
            o->state = (character_state)S_fly_run_jump;
            break;
          case run_jump_fall:
            o->state = (character_state)S_fly_run_jump_fall;
            break;
          case end_run_jump:
            o->state = (character_state)S_fly_end_run_jump;
            break;
          default:
            break;
        }
...
void *sgun_ai()
{
  ...
  if (whit
      || (who && figures[who->otype]->get_cflag(CFLAG_UNACTIVE_SHIELD) && who->total_objects()
...
void *score_draw()
{
  ...
  if (local)
  {
    ...
    {
      int color
          = lnumber_value(((LArray *)((LSymbol *)l_player_text_color)->GetValue())->Get(sorted_players[i]->get_tint()));
...
void *show_kills()
{
  ...
  main_screen->WidgetBar(ivec2(x, y + 2), ivec2(x + strlen(header_str) * fnt->Size().x, y + fnt->Size().y - 3),
