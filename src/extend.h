...
public:
  ...
  simple_object();
  int32_t x, y, last_x, last_y; // used for frame interpolation on fast machines
  ...
  void set_xvel(int32_t xv)
  {
    Xvel = xv;
  }
  void set_yvel(int32_t yv)
  {
    Yvel = yv;
  }
  void set_xacel(int32_t xa)
  {
    Xacel = xa;
  }
  void set_yacel(int32_t ya)
  {
    Yacel = ya;
  }
  void set_fx(uint8_t x)
  {
    Fx = x;
  }
  void set_fy(uint8_t y)
  {
    Fy = y;
  }
  void set_fxvel(uint8_t xv)
  {
    Fxvel = abs(xv);
  }
  void set_fyvel(uint8_t yv)
  {
    Fyvel = abs(yv);
  }
  void set_fxacel(uint8_t xa)
  {
    Fxacel = abs(xa);
  }
  void set_fyacel(uint8_t ya)
  {
    Fyacel = abs(ya);
  }
  void set_aitype(uint8_t t)
  {
    Aitype = t;
  }
  void set_aistate(uint16_t s)
  {
    Aistate = s;
  }
  void set_aistate_time(uint16_t t)
  {
    Aistate_time = t;
  }
  void set_hp(uint16_t h)
  {
    Hp = h;
  }
  void set_mp(uint16_t m)
  {
    Mp = m;
  }
  void set_fmp(uint16_t m)
  {
    Fmp = m;
  }

  void set_fade_count(uint8_t f)
  {
    Fade_count = f;
  }
  void set_fade_max(uint8_t m)
  {
    Fade_max = m;
  }
  void set_fade_dir(int8_t d)
  {
    Fade_dir = d;
  }

  void set_frame_dir(int8_t d)
  {
    Frame_dir = d;
  }
  ...
  void set_controller(view *v)
  {
    Controller = v;
  }

  void set_gravity(int x)
  {
    grav_on = x;
  }
