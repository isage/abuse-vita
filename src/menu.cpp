...
// percent is 0..256
static void TintArea(ivec2 aa, ivec2 bb, int r_to, int g_to, int b_to, int percent)
{
  ivec2 caa, cbb;
...
char *men_str(void *arg)
{
  switch (item_type(arg))
  {
    case L_STRING:
    {
      return lstring_value(arg);
    }
    break;
    case L_CONS_CELL:
    {
      return lstring_value(CAR(arg));
    }
    break;
    default:
    {
...
int menu(void *args, JCFont *font) // reurns -1 on esc
{
  ...
  int mx = main_screen->Size().x / 2 - mw / 2, my = main_screen->Size().y / 2 - mh / 2;
  ...
  if (title)
  {
    ...
    DarkWidget(ivec2(tx - 2, my - font->Size().y - 4), ivec2(tx + tl + 3, my - 1), wm->medium_color(), wm->dark_color(),
               180);
    font->PutString(main_screen, ivec2(tx + 1, my - font->Size().y - 2), title, wm->bright_color());
  }

  DarkWidget(ivec2(mx, my), ivec2(mx + mw, my + mh), wm->medium_color(), wm->dark_color(), 200);
  ...
  do
  {
    ...
    if (wm->IsPending())
    {
      ...
      {
        if (ev.mouse_move.x > mx && ev.mouse_move.x < mx + mw && ev.mouse_move.y > my && ev.mouse_move.y < my + mh)
        {
    ...
    if (!last_color_time || (int)(cur_time.diff_time(last_color_time) * 1000) > 120)
    {
      ...
      TintArea(ivec2(mx + 1, by1), ivec2(mx + mw - 1, by2 + 1), 63, 63, 63, color);
      ...
      main_screen->Rectangle(ivec2(mx + 1, by1), ivec2(mx + mw - 2, by2), wm->bright_color());
...
void show_sell(int abortable)
{
  ...
  if (!DEFINEDP(ss->GetValue()))
  {
    ...
    //    char *prog="((\"art/help.spe\" . \"sell2\")(\"art/help.spe\" . \"sell4\")(\"art/help.spe\" .
    //    \"sell3\")(\"art/fore/endgame.spe\" . \"credit\"))"; char *prog="((\"art/fore/endgame.spe\" . \"credit\")
    //    (\"art/help.spe\" . \"sell6\"))";
    ...
    set = new ico_switch_button(
        ...
        load_icon(
            ...
            load_icon(8, ID_MEDIUM, x, y, h,
                      load_icon(9, ID_HARD, x, y, h, load_icon(10, ID_EXTREME, x, y, h, NULL, "ic_extreme"), "ic_hard"),
                      "ic_medium"),
            "ic_easy"),
        NULL);
    y += h;
