...
char backw_on = 0, forew_on = 0, show_menu_on = 0, ledit_on = 0, pmenu_on = 0, omenu_on = 0, commandw_on = 0,
     tbw_on = 0, searchw_on = 0, small_render_on = 0, interpolate_draw = 0, disable_autolight = 0, fps_on = 0,
     profile_on = 0, show_names = 0, fg_reversed = 0, raise_all;
...
public:
  ...
  virtual int width()
  {
    return cache.img(id)->Size().x;
  }
  virtual int height()
  {
    return cache.img(id)->Size().y;
  }
};
...
int dev_del, dev_move, dev_char_left, dev_char_right, dev_back, dev_front, dev_ok, dev_copy, dev_brain, dev_lights,
    dev_objects, dev_ai, dev_mode_icon[DEV_MODES], dev_forward, dev_backward;

char const *dev_mode_icon_names[DEV_MODES] = {"pixel_mode", "pick_mode", /* "fill_mode",
                                              "line_mode","rect_mode","bar_mode", */
                                              "area_select"};
...
{
public:
  amb_cont(int X, int Y, ifield *Next)
      : scroller(X, Y, ID_NULL, 100, wm->font()->Size().y + 2, 0, 64, Next)
  {
    if (player_list)
...
int confirm_quit()
{
  ...
  if (settings.big_font)
    hr = (float)the_game->ar_big_font->Size().x / the_game->ar_small_font->Size().x;
  ...
  while (!fin)
  {
    ...
    if (ev.type == EV_MESSAGE && ev.message.id == ID_QUIT_OK)
      ...
      fin = 1;
    else if (ev.type == EV_KEY && toupper(ev.key) == toupper(*symbol_str("YES")))
      fin = quit = 1;
    else if (ev.type == EV_KEY && toupper(ev.key) == toupper(*symbol_str("NO")))
      fin = 1;
...
void dev_controll::make_ambient()
{
  ...
  ambw = wm->CreateWindow(ivec2(prop->getd("ambient x", -1), prop->getd("ambient y", -1)), ivec2(-1),
...
void scale_put(image *im, image *screen, int x, int y, short new_width, short new_height)
{
  ...
  int32_t xstep = (im->Size().x << 16) / new_width, ystep = (im->Size().y << 16) / new_height, iy, ix, sx, ix_start,
          iy_start;
  screen->AddDirty(ivec2(x, y), ivec2(x + new_width, y + new_height));

  ivec2 caa, cbb;
  screen->GetClip(caa, cbb);
  if (caa.x > cbb.x || caa.y > cbb.y || x >= cbb.x || y >= cbb.y || x + new_width <= caa.x || y + new_height <= caa.y)
    return;
  if (x < caa.x)
  {
...
void dev_controll::dev_draw(view *v)
{
  ...
  if (dev & EDIT_MODE)
  {
    ...
    if (dev & DRAW_LINKS)
    {
      ...
      {
        if (f->x - vx >= 0 && f->x - vx <= (v->m_bb.x - v->m_aa.x + 1) && f->y - vy >= 0
            && f->y - vy <= (v->m_bb.y - v->m_aa.y + 1))
        {
          image *im = cache.img(light_buttons[f->type]);
          main_screen->PutImage(im, ivec2(f->x - vx, f->y - vy) + v->m_aa - im->Size() / 2);
          main_screen->Rectangle(ivec2(f->x1 - vx, f->y1 - vy) + v->m_aa, ivec2(f->x2 - vx, f->y2 - vy) + v->m_aa,
                                 wm->medium_color());
        }
      }
    }
    ...
    if (selected_light)
    {
      ...
      main_screen->Rectangle(pos - im->Size() / 2, pos + im->Size() / 2, wm->bright_color());
    }
...
void dev_controll::toggle_toolbar()
{
  ...
  else
  {
    ...
    tool_picker *tp
        = new tool_picker(0, 0, ID_NULL, 5, (visual_object **)dev_mode_pict, dev_mode_ids, DEV_MODES, pal, pal, NULL);
    tbw = wm->CreateWindow(ivec2(prop->getd("toolbar x", -1), prop->getd("toolbar y", -1)), ivec2(-1), tp);
...
void dev_controll::toggle_show_menu()
{
  ...
  show_menu = wm->CreateWindow(ivec2(prop->getd("layer x", -1), prop->getd("layer y", -1)), ivec2(-1), fb,
                               symbol_str(symbol_str("SHOW?")));
}
...
void dev_controll::toggle_omenu()
{
  ...
  omenu = wm->CreateWindow(ivec2(prop->getd("objects x", 0), prop->getd("objects y", 0)), ivec2(-1),
                           new pick_list(0, 0, DEV_CREATE, yres / wm->font()->Size().y / 2, listable_objs,
                                         total_listable, 0, NULL, cache.img(window_texture)));
}
...
void dev_controll::toggle_pmenu()
{
  ...
  pmenu = wm->CreateWindow(ivec2(prop->getd("pal x", 0), prop->getd("pal y", -1)), ivec2(-1),
                           new pick_list(0, 0, DEV_PALETTE, yres / wm->font()->Size().y / 2, pwin_list, total_pals, 0,
...
void dev_controll::toggle_fgw()
{
  ...
  tile_picker *f_tp = new tile_picker(0, 0, DEV_FG_PICKER, SPEC_FORETILE, fg_scale, maxh, fg_w, NULL);
  f_tp->reverse();

  forew = wm->CreateWindow(ivec2(prop->getd("fore x", -30), prop->getd("fore y", 0)), ivec2(-1), f_tp,
                           symbol_str("l_fg"));
}
...
void dev_controll::toggle_bgw()
{
  ...
  /* FIXME: previous code had 1 instead of 0, investigate */
  tile_picker *f_tp = new tile_picker(0, 0, DEV_BG_PICKER, SPEC_BACKTILE, bg_scale, maxh, bg_w, NULL);
  forew             = wm->CreateWindow(ivec2(prop->getd("back x", -30), prop->getd("back y", 0)), ivec2(-1), f_tp,
                           symbol_str("l_bg"));
}
...
void dev_controll::toggle_search_window()
{
  ...
  search_window = wm->CreateWindow(
      ivec2(prop->getd("searchw x", -30), prop->getd("searchw y", 0)), ivec2(-1),
      new text_field(
          0, 0, ID_SEARCH_TEXT, "object name>", "***************************", prop->get("search name", ""),
          new button(bw, wm->font()->Size().y + 5, ID_SEARCH_BACKWARD, cache.img(dev_backward),
                     new button(bw * 3, wm->font()->Size().y + 5, ID_SEARCH_FOREWARD, cache.img(dev_forward), NULL))),
      "SEARCH");
...
void dev_controll::do_command(char const *command, Event &ev)
{
  ...
  if (!strcmp(fword, "delete"))
  {
    ...
    else if (selected_light)
    {
      ...
      the_game->show_help(symbol_str("d_nosel"));
    the_game->need_refresh();
  }
  ...
  if (!strcmp(fword, "move"))
  {
    ...
    if (edit_object)
      ...
      the_game->show_help("No object selected");
  }
  if (!strcmp(fword, "move_light"))
  {
    ...
    if (edit_light)
      ...
      the_game->show_help("No light selected");
  }
  ...
  if (!strcmp(fword, "to_front"))
  {
    ...
    if (which)
      ...
      the_game->show_help(symbol_str("forward?"));
  }
  ...
  if (!strcmp(fword, "to_back"))
  {
    ...
    if (which)
      ...
      the_game->show_help(symbol_str("back?"));
  }
  ...
  if (!strcmp(fword, "set_aitype"))
  {
    ...
    if (which)
    {
      ...
      else
      {
        ...
        {
          case '0':
            which->change_aitype(0);
            break;
          case '1':
            which->change_aitype(1);
            break;
          case '2':
            which->change_aitype(2);
            break;
          case '3':
            which->change_aitype(3);
            break;
          case '4':
            which->change_aitype(4);
            break;
          case '5':
            which->change_aitype(5);
            break;
          case '6':
            which->change_aitype(6);
            break;
          case '7':
            which->change_aitype(7);
            break;
          case '8':
            which->change_aitype(8);
            break;
          case '9':
            which->change_aitype(9);
            break;
          case ')':
            which->change_aitype(10);
            break;
          case '!':
            which->change_aitype(11);
            break;
          case '@':
            which->change_aitype(12);
            break;
          case '#':
            which->change_aitype(13);
            break;
          case '$':
            which->change_aitype(14);
            break;
          case '%':
            which->change_aitype(15);
            break;
          case '^':
            which->change_aitype(16);
            break;
          case '&':
            which->change_aitype(17);
            break;
          case '*':
            which->change_aitype(18);
            break;
          case '(':
            which->change_aitype(19);
            break;
        }
      }
      the_game->need_refresh();
      ...
      the_game->show_help(symbol_str("aitype"));
  }
}
...
void dev_controll::toggle_light_window()
{
  ...
  lightw = wm->CreateWindow(
      ivec2(prop->getd("light create x", 0), prop->getd("light create y", 0)), ivec2(-1),
      new button_box(
          0, 0, DEV_LIGHT_BUTTON_BOX, 1,
          new button(
              bw * 0, bh * 0, DEV_LIGHT0, cache.img(light_buttons[0]),
              new button(
                  bw * 1, bh * 0, DEV_LIGHT1, cache.img(light_buttons[1]),
                  new button(
                      bw * 2, bh * 0, DEV_LIGHT2, cache.img(light_buttons[2]),
                      new button(
                          bw * 0, bh * 1, DEV_LIGHT3,
                          cache.img(light_buttons[3]),
                          new button(
                              bw * 1, bh * 1, DEV_LIGHT4,
                              cache.img(light_buttons[4]),
                              new button(bw * 2, bh * 1, DEV_LIGHT5, cache.img(light_buttons[5]),
                                         new button(bw * 0, bh * 2, DEV_LIGHT6, cache.img(light_buttons[6]),
                                                    new button(bw * 1, bh * 2, DEV_LIGHT7, cache.img(light_buttons[7]),
                                                               new button(
                                                                   bw * 2, bh * 2, DEV_LIGHT8,
                                                                   cache.img(light_buttons[8]),
                                                                   new button(bw * 0, bh * 3, DEV_LIGHT9,
                                                                              cache.img(light_buttons[9]),
                                                                              new button(bw * 1, bh * 3, DEV_AMBIENT,
                                                                                         cache.img(light_buttons[11]),
                                                                                         NULL))))))))))),
          new text_field(0, bh * 4, DEV_LIGHTW, "W ", "******", prop->getd("light create w", 0),
                         new text_field(0, bh * 4 + th * 1, DEV_LIGHTH, "H ", "******", prop->getd("light create h", 0),
  ...
  if (figures[o->otype]->total_fields)
  {
    ...
    for (i = 0; i < figures[o->otype]->total_fields; i++)
    {
      ...
      ifield *p = new text_field(wl, wh, ID_NULL, tmp, "######",
                                 (double)o->get_var_by_name(figures[o->otype]->fields[i]->real_name, er), NULL);
      if (last)
        last->next = p;
      else
        first = p;
    ...
    aiw = wm->CreateWindow(ivec2(prop->getd("ai x", 0), prop->getd("ai y", 0)), ivec2(-1),
                           new button(wl, owh - 20, DEV_AI_OK, cache.img(dev_ok), first), "ai");
  }
  else
  {
    aiw = wm->CreateWindow(
        ivec2(prop->getd("ai x", 0), prop->getd("ai y", 0)), ivec2(-1),
        new button(
            wl, wh - 20, DEV_AI_OK, cache.img(dev_ok),
            new text_field(
                wl, wh + th * 0, DEV_AI_XVEL, symbol_str("ai_xvel"), "#####", (double)o->xvel(),
                new text_field(
                    wl, wh + th * 1, DEV_AI_YVEL, symbol_str("ai_yvel"), "#####", (double)o->yvel(),
                    new text_field(
                        wl, wh + th * 2, DEV_AI_XACEL, symbol_str("ai_xacel"), "#####", (double)o->xacel(),
                        new text_field(
                            wl, wh + th * 3, DEV_AI_YACEL, symbol_str("ai_yacel"), "#####", (double)o->yacel(),
                            new text_field(
                                wl, wh + th * 4, DEV_AI_STTIME, symbol_str("ai_stime"), "####",
                                (double)o->aistate_time(),
                                new text_field(
                                    wl, wh + th * 5,
                                    DEV_AI_GRAVITY, symbol_str("ai_gravity"), "####", (double)o->gravity(),
                                    new text_field(
                                        wl, wh + th * 6, DEV_AI_HEALTH, symbol_str("ai_health"), "####",
                                        (double)o->hp(),
                                        new text_field(wl, wh + th * 7, DEV_AI_MORPHPR, symbol_str("ai_morph"), "####",
                                                       (double)o->mp(),
                                                       new text_field(
                                                           wl, wh + th * 8, DEV_AI_TYPE, symbol_str("ai_type"), "####",
                                                           (double)o->aitype(),
                                                           new text_field(
                                                               wl, wh + th * 9, DEV_AI_STATE, symbol_str("ai_state"),
                                                               "####", (double)o->aistate(),
                                                               new text_field(wl, wh + th * 10, DEV_AI_FADE,
                                                                              symbol_str("ai_fade"), "####",
                                                                              (double)o->fade_count(), NULL)))))))))))),
        "ai");
  }
...
void dev_controll::close_ai_window()
{
  if (aiw)
  {
    ...
    if (o)
    {
      ...
      {
        x = atoi(aiw->read(DEV_AI_XVEL));
        if (x != o->xvel())
          o->set_xvel(x);
        x = atoi(aiw->read(DEV_AI_YVEL));
        if (x != o->yvel())
          o->set_yvel(x);

        x = atoi(aiw->read(DEV_AI_XACEL));
        if (x != o->xacel())
          o->set_xacel(x);
        x = atoi(aiw->read(DEV_AI_YACEL));
        if (x != o->yacel())
          o->set_yacel(x);

        x = atoi(aiw->read(DEV_AI_STTIME));
        if (x != o->aistate_time())
          o->set_aistate_time(x);
        x = atoi(aiw->read(DEV_AI_GRAVITY));
        if (x != o->gravity())
          o->set_gravity(x);

        x = atoi(aiw->read(DEV_AI_HEALTH));
        if (x != o->hp())
          o->set_hp(x);
        x = atoi(aiw->read(DEV_AI_MORPHPR));
        if (x != o->mp())
          o->set_mp(x);

        x = atoi(aiw->read(DEV_AI_TYPE));
        if (x != o->aitype())
          o->set_aitype(x);
        x = atoi(aiw->read(DEV_AI_STATE));
        if (x != o->aistate())
          o->set_aistate(x);
        x = atoi(aiw->read(DEV_AI_FADE));
        if (x != o->fade_count())
          o->set_fade_count(x);
      }
    }
...
void dev_controll::area_handle_input(Event &ev)
{
  ...
  if (ev.type == EV_MOUSE_BUTTON && ev.mouse_button)
  {
    ...
    if (!current_level)
      return;
    current_area = current_level->area_list = new area_controller(pos.x, pos.y, the_game->ftile_width(),
                                                                  the_game->ftile_height(), current_level->area_list);
    the_game->need_refresh();
...
void dev_controll::pick_handle_input(Event &ev)
{
  ...
  if (ev.type == EV_MOUSE_BUTTON && ev.mouse_button)
  {
    ...
    if (find && current_area && dc)
    {
      ...
      area_win = wm->CreateWindow(
          ivec2(prop->getd("area_box x", 0), prop->getd("area_box y", 0)), ivec2(-1),
          new button(
              wl + bw * 0, wh - 8, DEV_AREA_OK, cache.img(dev_ok),
              new button(
                  wl + bw * 1, wh - 8, DEV_AREA_DELETE, cache.img(dev_del),

                  new text_field(
                      wl, wh + th * 1, DEV_AREA_AMBIENT, symbol_str("a_ambient"), "******", current_area->ambient,
                      new text_field(
                          wl, wh + th * 2, DEV_AREA_AMBIENT_SPEED, symbol_str("a_aspeed"), "******",
                          current_area->ambient_speed,
                          new text_field(
                              wl, wh + th * 3, DEV_AREA_VIEW_XOFF, symbol_str("a_view_xoff"), "******",
                              current_area->view_xoff,
                              new text_field(wl, wh + th * 4, DEV_AREA_VIEW_YOFF, symbol_str("a_view_yoff"), "******",
                                             current_area->view_yoff,
                                             new text_field(wl, wh + th * 5, DEV_AREA_VIEW_XOFF_SPEED,
                                                            symbol_str("a_view_xspd"), "******",
                                                            current_area->view_xoff_speed,
                                                            new text_field(wl, wh + th * 6, DEV_AREA_VIEW_YOFF_SPEED,
                                                                           symbol_str("a_view_yspd"), "******",
                                                                           current_area->view_yoff_speed, NULL)))))))));
    }
    else if (find)
    {
      current_area         = find;
          ...
          switch (ev.key)
          {
            case '+':
            {
              if (edit_light->type == 9)
              {
                ...
                {
                  edit_light->inner_radius++;
                  rd = 1;
                }
              }
              else
              {
                edit_light->outer_radius++;
                rd = 1;
              }
            }
            break;
      ...
      if (dev & EDIT_MODE)
      {
        ...
        if (ev.mouse_button)
        {
          if (selected_object)
          {
            ...
            oedit = wm->CreateWindow(
                ivec2(prop->getd("oedit x", 0), prop->getd("oedit y", 0)), ivec2(-1),
                new button_box(
                    0, 0, ID_NULL, 1,
                    new button(
                        bw * 0, 0, DEV_OEDIT_OK, cache.img(dev_ok),
                                   ...
                                   new button(bw * 2, 0, DEV_OEDIT_FRONT, cache.img(dev_front),
                                              new button(bw * 3, 0, DEV_OEDIT_BACK, cache.img(dev_back),
                                                         new button(bw * 4, 0, DEV_OEDIT_COPY, cache.img(dev_copy),
                                                                    new button(bw * 0, bh * 1, DEV_OEDIT_DELETE,
                                                                               cache.img(dev_del), NULL)))))),
                    new button(
                        bw * 5, bh * 0, DEV_OEDIT_AI, cache.img(dev_ai),

                        new button_box(
                            bw * 1, bh * 1, DEV_OEDIT_CHAR_BOX, 0,
          ...
          else if (selected_light)
          {
            ...
            ledit = wm->CreateWindow(
                ivec2(prop->getd("ledit x", 0), prop->getd("ledit y", 0)), ivec2(-1),
                new button_box(
                    0, 0, ID_NULL, 1,
  ...
  switch (ev.type)
  {
    case EV_MESSAGE:
    {
      switch (ev.message.id)
      {
        ...
        case ID_LEVEL_NEW:
        {
          ...
          {
            mess_win
                = wm->CreateWindow(ivec2(xres / 2, yres / 2), ivec2(-1),
                                   new button(10, 20, ID_LEVEL_NEW_OK, symbol_str("YES"),
                                              new button(40, 20, ID_CANCEL, symbol_str("NO"),
                                                         new info_field(0, 0, ID_NULL, symbol_str("sure?"), NULL))),
                                   symbol_str("New?"));
            wm->grab_focus(mess_win);
          }
        }
        ...
        case ID_LEVEL_RESIZE:
        {
          if (!mess_win)
          {
            ...
            mess_win = wm->CreateWindow(
                ...
                new text_field(
                    0, h * 0, ID_MESS_STR1, symbol_str("width_"), "****",
                    ...
                    new text_field(0, h * 1, ID_MESS_STR2, symbol_str("height_"), "****",
                                   ...
                                   new button(10, h * 4, ID_LEVEL_RESIZE_OK, symbol_str("ok_button"),
                                              new button(40, h * 4, ID_CANCEL, symbol_str("cancel_button"), NULL)))),
                symbol_str("_scroll"));
          }
        }
        break;
        case ID_LEVEL_RESIZE_OK:
        {
          if (current_level)
          {
            current_level->set_size(atoi(mess_win->read(ID_MESS_STR1)), atoi(mess_win->read(ID_MESS_STR2)));
          }
          else
        ...
        case ID_RECORD_DEMO:
        {
          if (!mess_win)
          {
            ...
            mess_win = wm->CreateWindow(
                ivec2(xres / 2, yres / 2), ivec2(-1),
                new text_field(0, h * 0, ID_RECORD_DEMO_FILENAME, "demo filename", "*******************", "demo.dat",
        ...
        case ID_SET_SCROLL:
        {
          if (!mess_win)
          {
            ...
            mess_win = wm->CreateWindow(
                ivec2(xres / 2, yres / 2), ivec2(-1),
                new text_field(
                    0, h * 0, ID_MESS_STR1, symbol_str("x_mul"), "****", bg_xmul,
                    new text_field(
                        0, h * 1, ID_MESS_STR2, symbol_str("x_div"), "****", bg_xdiv,
                        new text_field(
                            0, h * 2, ID_MESS_STR3, symbol_str("y_mul"), "****", bg_ymul,
                            new text_field(
                                0, h * 3, ID_MESS_STR4, symbol_str("y_div"), "****", bg_ydiv,
        ...
        case ID_SET_SCROLL_CHECK:
        {
              ...
              || (((float)tbg_ymul / (float)tbg_ydiv) < ((float)bg_ymul / (float)bg_ydiv)))
          {
            ...
            warn_win = wm->CreateWindow(
                ivec2(xres / 2 - 40, yres / 2 - 40), ivec2(-1),
                new info_field(0, 0, ID_NULL, symbol_str("back_loss"),
        ...
        case ID_ADD_PALETTE:
        {
          if (!mess_win)
          {
            ...
            mess_win = wm->CreateWindow(
                ivec2(xres / 2, yres / 2), ivec2(-1),
                new text_field(
                    0, h * 0, ID_MESS_STR1, symbol_str("ap_width"), "****", 2,
                    new text_field(0, h * 1, ID_MESS_STR2, symbol_str("ap_height"), "****", 2,
                                   new text_field(0, h * 2, ID_MESS_STR3, symbol_str("ap_name"), "***********", "pal",
                                                  new button(10, h * 3, ID_ADD_PALETTE_OK, symbol_str("ok_button"),
                                                             new button(40, h * 3, ID_CANCEL,
                                                                        symbol_str("cancel_button"), NULL))))),
                symbol_str("ap_pal"));
          }
        }
        break;
        ...
        {
          char name[70];
          sprintf(name, "(add_palette \"%s\" %d %d)", mess_win->read(ID_MESS_STR3), atoi(mess_win->read(ID_MESS_STR1)),
    ...
    case EV_CLOSE_WINDOW:
    {
      if (ev.window)
      {
        ...
        else if (ev.window == backw)
          toggle_bgw();
        else if (ev.window == forew)
          toggle_fgw();
        else if (ev.window == lightw)
          toggle_light_window();
        else if (ev.window == show_menu)
          toggle_show_menu();
        else if (ev.window == pmenu)
          toggle_pmenu();
        else if (ev.window == tbw)
          toggle_toolbar();
        else if (ev.window == omenu)
          toggle_omenu();
        else if (ev.window == search_window)
          toggle_search_window();
    ...
    case EV_KEY:
    {
      ...
      if (ev.window == NULL || ev.window == pmenu || ev.window == forew || is_pal_win(ev.window)) // main window actions
      {
        switch (ev.key)
        {
          case JK_CTRL_L:
...
pal_win::pal_win(void *args)
{
  ...
  if (!NILP(ao))
  {
    w  = lnumber_value(CAR(ao));
    ao = CDR(ao);
    if (!NILP(ao))
    {
      h  = lnumber_value(CAR(ao));
      ao = CDR(ao);
      if (!NILP(ao))
      {
        x  = lnumber_value(CAR(ao));
        ao = CDR(ao);
        if (!NILP(ao))
        {
          y  = lnumber_value(CAR(ao));
          ao = CDR(ao);
          if (!NILP(ao))
            scale = lnumber_value(CAR(ao));
          ao = CDR(ao);
        }
      }
    }
...
void pal_win::open_window()
{
  if (me)
    close_window();
  me = wm->CreateWindow(ivec2(x, y), ivec2(w * f_wid / scale, h * f_hi / scale), NULL, name);
  draw();
}
...
void pal_win::draw()
{
  ...
  if (me)
  {
    ...
    for (i = 0; i < w * h; i++)
    {
      ...
      scale_put(im, me->m_surf, me->x1() + (i % w) * tw, me->y1() + (i / w) * th, tw, th);
      if (d == pat[i])
      {
        me->m_surf->Rectangle(ivec2(me->x1() + (i % w) * tw, me->y1() + (i / w) * th),
                              ivec2(me->x1() + (i % w) * tw + tw - 1, me->y1() + (i / w) * th + th - 1),
                              wm->bright_color());
      }
    }
    delete im;
      ...
      case EV_MOUSE_BUTTON:
      {
        if (ev.mouse_button == 1)
        {
          ...
          if (selx >= 0 && sely >= 0 && selx < w && sely < h)
          {
            ...
            if (dev_cont->forew)
              ((tile_picker *)dev_cont->forew->read(DEV_FG_PICKER))->recenter(dev_cont->forew->m_surf);
          }
        }
            ...
            for (xx = tile.x; xx < tile.x + w; xx++)
            {
              for (yy = tile.y; yy < tile.y + h; yy++)
              {
                if (xx >= 0 && yy >= 0 && xx < current_level->foreground_width()
                  ...
                  the_game->PutFg(ivec2(xx, yy), raise_all ? make_above_tile(pat[xx - tile.x + (yy - tile.y) * w])
                                                           : pat[xx - tile.x + (yy - tile.y) * w]);
              }
            }
          }
          break;
...
void dev_controll::fg_fill(int color, int x, int y, pal_win *p)
{
  ...
  do
  {
    ...
    {
      while (x > 0 && fgvalue(sl[x]) == fcolor)
        x--;
      if (fgvalue(sl[x]) != fgvalue(fcolor) && x < current_level->foreground_width() - 1)
        x++;
      if (y > 0)
      {
...
static pmi editmenu[] = {{"menu2_light", ID_TOGGLE_LIGHT, NULL, -1},
...
// Window Menus
static pmi winmenu[] = {{"menu3_fore", ID_WIN_FORE, &forew_on, -1},
...
static pmenu_item *i_recurse(pmi *first)
{
  if (first->id == -1)
    ...
    return new pmenu_item(first->id, first->name ? symbol_str(first->name) : 0, first->on_off, first->key,
                          i_recurse(first + 1));
}
  ...
  return new pmenu(x, y,
                   new pmenu_item(symbol_str("file_top"), new psub_menu(i_recurse(filemenu), NULL),
                                  new pmenu_item(symbol_str("edit_top"), new psub_menu(i_recurse(editmenu), NULL),
                                                 new pmenu_item(symbol_str("window_top"),
                                                                new psub_menu(i_recurse(winmenu), NULL), NULL))),
                   main_screen);
}
...
void toggle_edit_mode()
{
  ...
  {
    if (dev & MAP_MODE)
      dev -= MAP_MODE; // no map mode while playing!
...
{
  if (state == DEV_MOVE_LIGHT || state == DEV_MOVE_OBJECT || mouse_scrolling)
    return 1;
  else
    return 0;
}
