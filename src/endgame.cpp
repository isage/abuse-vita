              ...
              int mask_height, int xoff, int coff)
{
  ...
  for (; y < mask_height; y++)
  {
    ...
    for (; x < n->size; x++, sl++, rem++, l++)
    {
      ...
      int r3 = r1 + (r2 - r1) * fade256 / 256, g3 = g1 + (g2 - g1) * fade256 / 256, b3 = b1 + (b2 - b1) * fade256 / 256;
...
public:
  ...
  int x, y;
  ex_char *next;
  ex_char(int X, int Y, int Frame, int Char_num, ex_char *Next)
  {
    x        = X;
    y        = Y;
    frame    = Frame;
...
void show_end2()
{
  ...
  {
    char nm[100];
    sprintf(nm, "small_wite%04d.pcx", i + 1);
  ...
  {
    char nm[100];
    sprintf(nm, "small_fire%04d.pcx", i + 1);
  ...
  for (i = 0; i < 80;)
  {
    ...
    if (new_time.diff_time(&old_time) > 0.1)
    {
      ...
      if (i >= 30 && i <= 37)
      {
        ...
        scan_map(main_screen, ex, ey, tcopy, cache.img(planet2), 0, paddr, p, cache.img(mask)->Size().y, eoff, coff);
      }
      else
        scan_map(main_screen, ex, ey, cache.img(planet), cache.img(planet2), 0, paddr, p, cache.img(mask)->Size().y,
                 eoff, coff);
      if (i > 38)
      {
        int t    = i - 38;
        image *s = cache.img(ship);
        int nw = s->Size().x * (t + 2) / 16, nh = s->Size().y * (t + 2) / 16;
  ...
  for (i = 0; i < 320 && ev.type != EV_KEY; i++)
  {
    ...
    scan_map(main_screen, ex, ey, cache.img(planet), cache.img(planet2), 256, paddr, p, cache.img(mask)->Size().y, eoff,
             coff);
...
void share_end()
{
  ...
  for (i = 0; i < 320 && ev.type != EV_KEY; i++)
  {
    ...
    text_draw(205 - i, dx + 10, dy, dx + 319 - 10, dy + 199, lstring_value(mid_plot), wm->font(), cmap,
              wm->bright_color());
    wm->flush_screen();
    time_marker now;
    while (now.diff_time(&start) < 0.18)
...
{
  // AR real end screen
  // since there is a victory.hmi in the music folder my guess would be that it was supposed to be played during the end
  // screen you can even hear him howling in the track, it matches the on screen text
  ...
  if (!settings.hires)
    im = cache.img(cache.reg("art/fore/endgame.spe", "end.pcx", SPEC_IMAGE, 1));
  else
    im = cache.img(cache.reg("art/fore/endgame.spe", "end.pcx_hires", SPEC_IMAGE, 1));
