...
void Game::grow_views(int amount)
{
  ...
  for (f = first_view; f; f = f->next)
  {
    ...
    if (f->suggest.cx2 - f->suggest.cx1 < 20 || f->suggest.cy2 - f->suggest.cy1 < 15 || f->suggest.cx1 < 0
        || f->suggest.cy1 < 0)
      f->suggest.send_view = 0;

    if (f->next && f->next->local_player() && f->suggest.cy2 >= f->next->m_aa.y)
      f->suggest.send_view = 0;
  }
}
...
ivec2 Game::MouseToGame(ivec2 pos, view *v)
{
  ...
  if (dev & MAP_MODE)
    return ivec2((pos.x - v->m_aa.x) * ftile_width() / AUTOTILE_WIDTH + map_xoff * ftile_width(),
                 (pos.y - v->m_aa.y) * ftile_height() / AUTOTILE_HEIGHT + map_yoff * ftile_height());
...
ivec2 Game::GameToMouse(ivec2 pos, view *v)
{
  ...
  if (dev & EDIT_MODE)
    ...
    tmp = ivec2(v->m_focus->x / ftile_width() - (v->m_bb.x - v->m_aa.x) / AUTOTILE_WIDTH / 2,
                v->m_focus->y / ftile_height() - (v->m_bb.y - v->m_aa.y) / AUTOTILE_HEIGHT / 2);
  else
    tmp = ivec2(0, 0);
  ...
  if (tmp.x > 0)
    ret.x -= (v->m_focus->x * AUTOTILE_WIDTH / ftile_width()) % AUTOTILE_WIDTH;
  if (tmp.y > 0)
    ret.y -= (v->m_focus->y * AUTOTILE_HEIGHT / ftile_height()) % AUTOTILE_HEIGHT;
...
void Game::draw_value(image *screen, int x, int y, int w, int h, int val, int max)
{
  ...
  screen->Bar(ivec2(x, y + 1), ivec2(x + w * val / max, y + h - 1), wm->bright_color());
}
...
void Game::dev_scroll()
{
  ...
  if (dev)
  {
    ...
    int xs, ys;
    if (mousex < xmargin && dev_cont->ok_to_scroll())
      xs = -18;
    else if (mousex > (main_screen->Size().x - xmargin) && dev_cont->ok_to_scroll())
      xs = 18;
    ...
    if (mousey < ymargin && dev_cont->ok_to_scroll())
      ys = -18;
    else if (mousey > (main_screen->Size().y - ymargin) && dev_cont->ok_to_scroll())
      ys = 18;
...
void Game::draw_map(view *v, int interpolate)
{
  ...
  if (!(dev & MAP_MODE))
  {
          ...
          if (x < fg_w && y < fg_h)
          {
            ...
            if (fort_num != BLACK)
            {
              ...
              if (p->tot)
              {
                ...
                {
                  d += 2;
                  main_screen->Line(ivec2(draw_x + *(d - 2), draw_y + *(d - 1)), ivec2(draw_x + *d, draw_y + *(d + 1)),
                                    b);
                }
    ...
    // if(cache.in_use()) main_screen->PutImage(cache.img(vmm_image), ivec2(v->m_aa.x, v->m_bb.y -
    // cache.img(vmm_image)->Size().y+1));
...
{
  return (last_demo_mpos.x >= x1 && last_demo_mpos.x <= x2 && last_demo_mpos.y >= y1 && last_demo_mpos.y <= y2);
}
...
void do_title()
{
  ...
  if (!fp->open_failure())
  {
    ...
    while (ev.type != EV_KEY && ev.type != EV_MOUSE_BUTTON)
    {
      ...
      text_draw(205 * hr - i, dx + 15 * hr, dy, dx + 320 * hr - 15 * hr, dy + 199 * hr, str, wm->font(), cmap,
                wm->bright_color());
...
void Game::update_screen()
{
  ...
  else if (current_level)
  {
    if (!(dev & EDIT_MODE) || refresh)
    {
      ...
      for (; f; f = f->next)
      {
        if (f->m_focus)
        {
          ...
          total_active += current_level->add_drawables(f->xoff() - w / 4, f->yoff() - h / 4, f->xoff() + w + w / 4,
                                                       f->yoff() + h + h / 4);
        }
      }
    ...
    {
      for (view *f = first_view; f; f = f->next)
        main_screen->PutImage(cache.img(pause_image),
                              ivec2((f->m_aa.x + f->m_bb.x) / 2 - cache.img(pause_image)->Size().x / 2, f->m_aa.y + 5),
                              1);
    }
                  ...
                  case LOWER_MUSIC:
                  {
                    ...
                    /*                                        volume_window->inm->redraw();
                                                            draw_value(volume_window->m_surf, 2, 43,
                                                                    (volume_window->x2()-volume_window->x1()-1), 8,
                       sfx_volume, 127); draw_value(volume_window->m_surf, 2, 94,
                                                                    (volume_window->x2()-volume_window->x1()-1), 8,
                       music_volume, 127);
                    */
                    break;
    ...
    for (view *f = first_view; f; f = f->next)
    {
      if (f->m_focus)
      {
        ...
        if (settings.ctr_aim == 1)
        {
          ...
          wm->SetMousePos(ivec2(f->m_focus->x - f->xoff() + cos(angle) * settings.ctr_cd + settings.ctr_aim_correctx,
...
void Game::draw(int scene_mode)
{
  ...
  if (scene_mode)
  {
    ...
    wm->font()->PutString(main_screen, pos + ivec2(1), helpstr, wm->dark_color());
