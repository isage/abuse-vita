...
int level::add_actives(int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
  ...
  for (; o; o = o->next)
  {
    if (!o->active)
    {
      int32_t xr = figures[o->otype]->rangex, yr = figures[o->otype]->rangey;
...
int level::add_drawables(int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
  ...
  for (; o; o = o->next)
  {
    if (ft || !o->active)
    {
      int32_t xr = figures[o->otype]->draw_rangex, yr = figures[o->otype]->draw_rangey;
      ...
      if (o->x + xr >= x1 && o->x - xr <= x2 && o->y + yr >= y1 && o->y - yr <= y2)
      {
        ...
        o->active = 0; // if this is the first pass, then mark objects not in this ranges as not active
    }
  }
  if (last_active)
    last_active->next_active = NULL;
...
view *level::make_view_list(int nplayers)
{
  ...
  for (j = 0, o = first; o && j < nplayers; o = o->next)
  {
    if (!strcmp(object_names[o->otype], "START"))
    {
      f = new view(create(use_type, o->x, o->y), f, num);
      num++;
...
{
  if (subject->pushable() && target->pushable() && subject->state != dead && target->state != dead
      ...
      if (c)
      {
        ...
        if (c->local_player())
        {
          ...
          else if (shutdown_lighting_value != c->ambient) // do we need to lower light toward real ambient?
          {
            ...
            shutdown_lighting = 0; // back to normal
        }

        if (smallest)
          c->configure_for_area(smallest);

        o->move(c->x_suggestion, c->y_suggestion, c->b1_suggestion | (c->b2_suggestion << 1) | (c->b3_suggestion << 2));
          ...
          if (se)
          {
            ...
            else
            {
              ...
              for (; f; f = f->next)
              {
                ...
                {
                  case RC_8:
                    f->set_var(j, fp->read_uint8());
                    break;
                  case RC_16:
                    f->set_var(j, fp->read_uint16());
                    break;
                  case RC_32:
                    f->set_var(j, fp->read_uint32());
                    break;
                }
                ...
                if (j == frame_var)
                {
                  if (f->otype != 0xffff && f->current_frame >= figures[f->otype]->get_sequence(f->state)->length())
                    f->current_frame = 0;
                }
              }
            }
...
bFILE *level::create_dir(char *filename, int save_all, object_node *save_list, object_node *exclude_list)
{
  ...
  for (i = 0; i < TOTAL_OBJECT_VARS; i++)
    sd.add_by_hand(new spec_entry(SPEC_DATA_ARRAY, object_descriptions[i].name, NULL,
                                  1 + RC_type_size(object_descriptions[i].type) * t, 0));
...
void level::write_thumb_nail(bFILE *fp, image *im)
{
  ...
  if (first_name)
    the_game->save_game_font->PutString(i, ivec2(80 - strlen(first_name) * the_game->save_game_font->Size().x / 2, 100),
                                        first_name);
...
int level::load_player_info(bFILE *fp, spec_directory *sd, object_node *save_list)
{
  ...
  if (se)
  {
    ...
    if (the_game->first_view == player_list)
      set_first_view = 1;
  ...
  for (i = 0; i < default_simple.total_vars(); i++)
  {
    ...
    for (o = save_list; o; o = o->next)
    {
      switch (t)
      {
        case RC_8:
        {
          fp->write_uint8(o->me->get_var(i));
        }
        break;
        case RC_16:
        {
          fp->write_uint16(o->me->get_var(i));
        }
        break;
        case RC_32:
        {
          fp->write_uint32(o->me->get_var(i));
        }
        break;
      }
    }
...
void level::load_links(bFILE *fp, spec_directory *sd, object_node *save_list, object_node *exclude_list)
{
    ...
    if (fp->read_uint8() == RC_32)
    {
      ...
      while (t)
      {
        ...
        if (x2 > 0)
          ...
          p = number_to_object_in_list(-x2, exclude_list);
        if (q)
          q->add_object(p);
        else
...
void level::remove_object(game_object *who)
{
  ...
  else
  {
    ...
    if (o)
    {
      ...
      return; // if object is not in level, don't try to do anything else
  }
  total_objs--;
...
void level::foreground_intersect(int32_t x1, int32_t y1, int32_t &x2, int32_t &y2)
{
  ...
  int32_t tl = the_game->ftile_width(), th = the_game->ftile_height(), j, xp1, yp1, xp2,
          yp2, // starting and ending points of block line segment
      swap;    // temp var
  ...
  if (blockx2 >= foreground_width())
  {
    x2 = tl * foreground_width() - 1;
  }
  if (blocky2 >= foreground_height())
  {
...
{
  int32_t tl = f_wid, th = f_hi, j, xp1, yp1, xp2, yp2; // starting and ending points of block line segment temp var
...
game_object *level::get_random_start(int min_player_dist, view *exclude)
{
  ...
  game_object *o = first;
  for (; o; o = o->next)
    if (o->otype == start_position_type)
      t++; // count how many starts there are in the level

  if (t == 0)
    return NULL; // there aren't any starts in level!
...
game_object *level::find_object_in_angle(int32_t x, int32_t y, int32_t start_angle, int32_t end_angle, void *list,
                                         game_object *exclude)
{
  game_object *closest     = NULL;
  int32_t closest_distance = 0xfffffff, distance, xo, yo;
