...
static unsigned char *cstart, *cend, *collected_start, *collected_end;
...
LObject *Lisp::CollectObject(LObject *x)
{
    ...
    if ((unsigned char *)x >= cstart && (unsigned char *)x < cend)
    {
        switch (item_type(x))
        {
        case L_BAD_CELL:
    ...
    else if ((unsigned char *)x < collected_start || (unsigned char *)x >= collected_end)
    {
...
void Lisp::CollectSpace(LSpace *which_space, int grow)
{
    ...
    unsigned char *new_data = (unsigned char *)malloc(LSpace::Gc.m_size);
