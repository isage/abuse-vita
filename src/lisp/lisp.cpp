...
LFixedPoint *LFixedPoint::Create(int x)
{
...
LChar *LChar::Create(unsigned short ch)
{
...
int lnumber_value(void *lnumber)
{
    ...
    case L_STRING:
        return (unsigned char)*lstring_value(lnumber);
    case L_CHARACTER:
...
void LObject::Print()
{
    ...
    case L_CHARACTER:
        if (current_print_file)
        {
            unsigned char ch = ((LChar *)this)->m_ch;
            current_print_file->write(&ch, 1);
        }
        else
        {
            unsigned short ch = ((LChar *)this)->m_ch;
            dprintf("#\\");
            switch (ch)
            {
            case '\n':
...
LObject *LSysFunction::EvalFunction(LList *arg_list)
{
    ...
    case SYS_FUNC_MINUS:
    {
        int sub = lnumber_value(CAR(arg_list)->Eval());
        arg_list = (LList *)CDR(arg_list);
        while (arg_list)
        {
    ...
    case SYS_FUNC_GT:
    {
        int n1 = lnumber_value(CAR(arg_list)->Eval());
        int n2 = lnumber_value(CAR(CDR(arg_list))->Eval());
    ...
    {
        char tmp[50], *tp;
        int num = lnumber_value(CAR(arg_list)->Eval());
        int dig = lnumber_value(CAR(CDR(arg_list))->Eval());
        tp = tmp + 49;
        *(tp--) = 0;
        while (num)
        {
    ...
    case SYS_FUNC_MIN:
    {
        int x = lnumber_value(CAR(arg_list)->Eval());
        int y = lnumber_value(CAR(CDR(arg_list))->Eval());
    ...
    case SYS_FUNC_ATAN2:
    {
        int y = (lnumber_value(CAR(arg_list)->Eval()));
        int x = (lnumber_value(CAR(CDR(arg_list))->Eval()));
    ...
    {
        int first = lnumber_value(CAR(arg_list)->Eval());
        arg_list = (LList *)CDR(arg_list);
        while (arg_list)
        {
    ...
    {
        int l = lnumber_value(CAR(arg_list)->Eval());
        if (l >= (2 << 16) || l <= 0)
        {
    ...
    {
        int x1 = lnumber_value(CAR(arg_list)->Eval());
        int x2 = lnumber_value(CAR(CDR(arg_list))->Eval());
        LObject *st = CAR(CAR(CDR(arg_list)))->Eval();
        PtrRef r1(st);

        if (x1 < 0 || x1 > x2 || x2 >= (int)strlen(lstring_value(st)))
...
void Lisp::Init()
{
    ...
    LSpace::Tmp.m_free = LSpace::Tmp.m_data = (unsigned char *)malloc(0x1000);
    LSpace::Tmp.m_size = 0x1000;
    LSpace::Tmp.m_name = "temporary space";

    LSpace::Perm.m_free = LSpace::Perm.m_data = (unsigned char *)malloc(0x1000);
